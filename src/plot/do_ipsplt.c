/*
 * $Id: do_ipsplt.c,v 1.3 2004/06/16 21:25:54 mbarnes Exp $
 */

/* ionic pseudo potential plotting with xmgrace */
...
#include "do_ipsplt.h"           /* the module's own header */
#include "nlm.h"

int do_ipsplt(param_t *param, char *logfile){
  ...
  sprintf(comm, "xmgrace %s.plt_ips -p ips.par -saveall %s_ips.agr & ", param->name,param->name);
  system(comm);
  free(comm);
