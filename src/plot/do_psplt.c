/*
 * $Id: do_psplt.c,v 1.3 2004/06/16 21:25:54 mbarnes Exp $
 */
...
#include "do_psplt.h"           /* the module's own header */
#include "nlm.h"

int do_psplt(param_t *param, char *logfile){
  ...
  sprintf(comm, "xmgrace %s.plt_sps -p sps.par -saveall %s.IPSPLOT & ", param->name,param->name);
  system(comm);
  free(comm);
